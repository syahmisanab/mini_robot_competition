...
def victim_rescuer(rescue_flag, exit_flag, terminate_flag):
    ...
    # Track how many balls have been dropped
    num_alive_dropped = 0
    num_dead_dropped = 0
    MAX_ALIVE = 2
    MAX_DEAD = 1

    # === Movement swap for back camera ===
    def cam_forward(speed=100): backward(speed)
    def cam_backward(speed=100): forward(speed)
    def cam_left(speed=150): right(speed)
    def cam_right(speed=150): left(speed)
    def cam_spin_left(speed=80): spin_right(speed)
    ...
    while True:
        ...
        if not holding_ball:
            # Stop if all balls have been dropped
            if num_alive_dropped >= MAX_ALIVE and num_dead_dropped >= MAX_DEAD:
                current_action = "All balls dropped. Switching to exit."
                rescue_flag.value = False
                exit_flag.value = True
                break
            # ...existing code for ball detection and movement...
            # Only chase balls if quota not reached
            filtered_ball_info = []
            for info in detected_ball_info:
                _, _, label, *_ = info
                if label == "silver_ball" and num_alive_dropped < MAX_ALIVE:
                    filtered_ball_info.append(info)
                elif label != "silver_ball" and num_dead_dropped < MAX_DEAD:
                    filtered_ball_info.append(info)
            if filtered_ball_info:
                ...
                if target_cy > proximity_y_threshold: # Ball is close enough to attempt grip/release
                    ...
                    cam_brake() # Stop to prepare for gripping
